      ...
      <nav className="bg-transparent border-gray-200 ">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4 pr-2 md:p-3 md:pr-1">
          ...
          <div className={`items-center flex md:order-2 justify-center text-base `}>
            <div className={` ${token === true ? 'flex' : 'hidden'}   items-center space-x-2 sm:space-x-5 fontM mr-3`} >
              ...
              <Link href={'/profile'} className='fontM hover:bg-sky-800 text-white bg-blue-500 py-1 px-3 rounded-md'>Profile</Link>
              <Image className={`w-9 h-9 rounded-full object-cover ${token === true ? 'inline-block' : 'hidden'}`} src={profile} alt="user photo" />
            </div>
            {/* if logout */}

            <div className={` ${token === false ? 'flex' : 'hidden'}   items-center space-x-2 sm:space-x-5 fontM md:mr-3`} >
            ...
            <button data-collapse-toggle="navbar-user" type="button" onClick={showmenu} className={` ${token === true ? 'flex' : 'hidden'} inline-flex items-center p-2  w-10 h-10 justify-center text-sm text-white rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600`} aria-controls="navbar-user" aria-expanded="false">
              <span className="sr-only">Open main menu</span>
              <svg className={`w-5 h-5 ${token === true ? 'inline-block' : 'hidden'}`} aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 17 14">
