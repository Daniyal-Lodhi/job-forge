...
const page = () => {
  ...
  const signin = (e)=>{
    ...
    // console.log(Object.keys(formErrors).length===0?formData:formErrors)
    if(Object.keys(formErrors).length!==0){
      console.log('plz fill up the form properly')
    }
    else{
      axios.post('/api/seeker/login',formData).
      then(res=>{
            ...
            <p tabIndex="0" className="focus:outline-none text-sm mt-4 font-medium leading-none text-gray-500">Dont have account? <a href="javascript:void(0)" className="hover:text-gray-500 focus:text-gray-500 focus:outline-none focus:underline hover:underline text-sm font-medium leading-none  text-gray-800 cursor-pointer"> Sign up here</a></p>
              ...
              <div className="mt-6  w-full">
                ...
                <div className="relative flex items-center justify-center">
                  <input onChange={handleChange} id="pass" type="password" className="bg-gray-200 border rounded  text-xs font-medium leading-none text-gray-800 py-3 w-full pl-3 mt-2" name='password' />
              ...
              <div className="mt-8">
                <button onClick={signin}  id='signinBtn' className=" disabled:bg-slate-500 focus:ring-2 focus:ring-offset-2 focus:ring-indigo-700 text-sm font-semibold leading-none text-white focus:outline-none bg-indigo-700 border rounded hover:bg-indigo-600 py-4 w-full">Sign in</button>
